...
inline void runTest(DiscreteContactManager& checker, const std::string& file_path)
{
  ...
  for (auto& r : result_vector)
  {
    ...
    mesh_vertices_color[static_cast<std::size_t>((*mesh_triangles)[static_cast<std::size_t>(4 * r.subshape_id[static_cast<std::size_t>(idx)] + 1)])] =
        Eigen::Vector3i(255, 0, 0);
    mesh_vertices_color[static_cast<std::size_t>((*mesh_triangles)[static_cast<std::size_t>(4 * r.subshape_id[static_cast<std::size_t>(idx)] + 2)])] =
        Eigen::Vector3i(255, 0, 0);
    mesh_vertices_color[static_cast<std::size_t>((*mesh_triangles)[static_cast<std::size_t>(4 * r.subshape_id[static_cast<std::size_t>(idx)] + 3)])] =
        Eigen::Vector3i(255, 0, 0);
  }
