...
typedef std::shared_ptr<trajopt::TermInfo> TermInfoPtr;
class TrajOptProb;
typedef std::shared_ptr<trajopt::TrajOptProb> TrajOptProbPtr;
struct ProblemConstructionInfo;
struct TrajOptResult;
typedef std::shared_ptr<trajopt::TrajOptResult> TrajOptResultPtr;

TrajOptProbPtr ConstructProblem(const ProblemConstructionInfo&);
TrajOptProbPtr ConstructProblem(const Json::Value&, tesseract::BasicEnvConstPtr env);
TrajOptResultPtr OptimizeProblem(TrajOptProbPtr, const tesseract::BasicPlottingPtr plotter = nullptr);
...
public:
  ...
  int GetNumSteps();  
  int GetNumDOF();
  tesseract::BasicKinConstPtr GetKin();
  tesseract::BasicEnvConstPtr GetEnv();
